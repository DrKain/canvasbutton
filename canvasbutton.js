...
function Button(ctx, options) {
    this.SHID = Math.random().toString(36).substr(2, 5);
    this.debug = false;

    __buttonGlobal[this.SHID] = {mo: false};
...
Button.prototype.hide = function(){ this.style._hidden = true; };
Button.prototype.show = function(){ this.style._hidden = false; };
...
Button.prototype.update = function (mouse, ctx) {
    ...
    if (mouse.x >= this.style.x && mouse.x <= (this.style.x + (this.style.width + (this.style.borderSize))) && mouse.y >= this.style.y && mouse.y <= (this.style.y + (this.style.height + (this.style.borderSize)))) {
        this.style.mouseOver = true;
        if (this.style.mouseOver !== ov)
            this.mouseenter();
        __buttonGlobal[this.SHID].mo = true;
    } else {
    ...
    if (Object.keys(__buttonGlobal).filter(function (i) {
            return __buttonGlobal[i].mo === true;
        }).length > 0) ctx.canvas.style.cursor = 'pointer';
...
Button.prototype.draw = function (ctx) {
    ...
    ctx.fillStyle = (this.style.mouseOver ? this.style.hover.backgroundColor : this.style.backgroundColor);
    ctx.beginPath();
    ctx.moveTo(this.style.x + this.style.radius, this.style.y);
    ctx.lineTo(this.style.x + this.style.width - this.style.radius, this.style.y);
    ctx.quadraticCurveTo(this.style.x + this.style.width, this.style.y, this.style.x + this.style.width, this.style.y + this.style.radius);
    ctx.lineTo(this.style.x + this.style.width, this.style.y + this.style.height - this.style.radius);
    ctx.quadraticCurveTo(this.style.x + this.style.width, this.style.y + this.style.height, this.style.x + this.style.width - this.style.radius, this.style.y + this.style.height);
    ctx.lineTo(this.style.x + this.style.radius, this.style.y + this.style.height);
    ctx.quadraticCurveTo(this.style.x, this.style.y + this.style.height, this.style.x, this.style.y + this.style.height - this.style.radius);
    ctx.lineTo(this.style.x, this.style.y + this.style.radius);
    ctx.quadraticCurveTo(this.style.x, this.style.y, this.style.x + this.style.radius, this.style.y);
    ctx.closePath();
    ctx.stroke();
    ctx.fill();
    ...
    ctx.fillStyle = (this.style.mouseOver ? this.style.hover.textColor : this.style.textColor);

    ctx.fillText(this.style.text, this.style.x + (this.style.width / 2), this.style.y + (this.style.height / 2));
};